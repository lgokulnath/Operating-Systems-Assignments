...
struct sleeplock lock_insert_sem;
struct sleeplock lock_delete_sem;
extern void forkret(void);
static void freeproc(struct proc *p);
struct barrier br[10];
struct buffer_cond buffer[20];
struct buffer_sem buffer_sem[20];
// Stats
  ...
  for (;;)
  {
    ...
    else
    {
      for (p = proc; p < &proc[NPROC]; p++)
      {
        ...
        // printf("Checking process %d on cpu %d\n", p->pid, cpuid());
        if (p->state == RUNNABLE)
        {
          ...
          // printf("Switching to process %d for cpu: %d tail: %d head: %d\n", p->pid, cpuid(),tail,head);
          // for(int g=0;g<20;g++){
          //   printf("%d(%d) ",buffer[g].x,buffer[g].full);
          // }
          // printf("\n");
          ...
          // printf("Ended process %d for cpu %d\n", p->pid, cpuid());
          // Process is done running for now.
          // It should have changed its p->state before coming back.
          c->proc = 0;
        }
...
void sleep(void *chan, struct spinlock *lk)
{
  ...
  // printf("Process %d going to sleep for cpu %d by sleep\n", p->pid, cpuid());
  sched();
...
void condsleep(struct cond_t *cv, struct sleeplock *lk)
{
  ...
  // printf("Process %d going to sleep for cpu %d by condsleep\n", p->pid, cpuid());
  sched();
  // printf("Process %d waking from sleep for cpu %d by condsleep\n", p->pid, cpuid());
  // Tidy up.
  p->chan = 0;
...
void barrier_init(void)
{
  ...
  {
    br[i].free = 1;
    initsleeplock(&br[i].blk,r);
  }
  initsleeplock(&print_lock, r);
...
int barrier_alloc(void)
{
  ...
  for (i = 0; i < 10; i += 1)
  {
    acquiresleep(&br[i].blk);
    if (br[i].free == 1)
    {
      br[i].free = 0;
      releasesleep(&br[i].blk);
      return i;
    }
    releasesleep(&br[i].blk);
  }
...
void barrier(int barrier_instance, int id, int num_proc)
{
  acquiresleep(&br[id].blk);
  br[id].num += 1;
  acquiresleep(&print_lock);
  printf("%d: Entered barrier#%d for barrier array id %d\n", myproc()->pid, barrier_instance, id);
  releasesleep(&print_lock);
  if (num_proc == br[id].num)
  {
    cond_broadcast(&br[id].cv);
    br[id].num = 0;
  }
  else
  {
    cond_wait(&br[id].cv, &br[id].blk);
  }
  acquiresleep(&print_lock);
  printf("%d: Finished barrier#%d for barrier array id %d\n", myproc()->pid, barrier_instance, id);
  releasesleep(&print_lock);
  releasesleep(&br[id].blk);
}
...
void buffer_cond_init(void)
{
  for (int i = 0; i < 20; i++)
  {
    buffer[i].full = 0;
    buffer[i].x = -1;
    initsleeplock(&buffer[i].buff_lock, "buffer_lock");
  }
  tail = 0;
  head = 0;
...
void cond_produce(int x)
{
  ...
  acquiresleep(&buffer[index].buff_lock);
  while (buffer[index].full)
  { condsleep(&buffer[index].deleted, &buffer[index].buff_lock);
  }
  buffer[index].x = x;
  buffer[index].full = 1;
  cond_signal(&buffer[index].inserted);
  releasesleep(&buffer[index].buff_lock);
}
...
int cond_consume(void)
{
  ...
  while (buffer[index].full==0){
    cond_wait(&buffer[index].inserted, &buffer[index].buff_lock);
  }
...
void sem_init(struct sem *s, int x)
{
  s->value = x;
  char name[] = "sem_lock";
  initsleeplock(&s->sem_lk, name);
}

void sem_wait(struct sem *s)
{
  acquiresleep(&s->sem_lk);
  while (s->value == 0)
    cond_wait(&s->sem_cv, &s->sem_lk);
  s->value--;
  releasesleep(&s->sem_lk);
}
